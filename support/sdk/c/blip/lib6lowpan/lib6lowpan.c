...
uint8_t *lowpan_unpack_headers(struct lowpan_reconstruct *recon,
  ...
  buf = unpack_address(&hdr->ip6_src,
                       ...
                       &frame->ieee_src,
                       frame->ieee_dstpan);
  if (!buf || (buf - unpack_start > cnt)) {
#if PRINTFUART_ENABLED
    printf("lowpan_unpack_headers: unpacked too much.1 %p %p %i", buf,
           unpack_start, cnt);
#endif
    return NULL;
  }
  ...
  if (!buf || (buf - unpack_start > cnt)) {
#if PRINTFUART_ENABLED
    printf("lowpan_unpack_headers: unpacked too much.2 %p %p %i", buf,
           unpack_start, cnt);
#endif
    return NULL;
  }
  ...
  if ((*dispatch) & LOWPAN_IPHC_NH_MASK) {
    ...
    if (!buf || (buf - unpack_start > cnt)) {
#if PRINTFUART_ENABLED
      printf("lowpan_unpack_headers: unpacked too much.3 %p %p %i", buf,
             unpack_start, cnt);
#endif
      return NULL;
    }
